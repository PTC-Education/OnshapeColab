...
def connectToOnshape(did, wid, eid, access, secret, base=None, verbose=False):
    setArgs(did, wid, eid, base=base, verbose=verbose)
    setKeys(access, secret)
    connectToClient(verbose=verbose)
...
def getAssemblyInfo(verbose=False):
    ...
    response = callAPI('assembly-definition', payload , params, True)
...
def postTransform(M, isRelative, parts, verbose):
    ...
    try:
        response = callAPI('occurrence-transforms', params, payload, False)
